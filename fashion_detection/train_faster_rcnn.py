...
os.environ['MXNET_GPU_MEM_POOL_RESERVE ']='99'
import logging
import time
import numpy as np
import mxnet as mx
...
'''
Namespace(batch_size=4, dataset='voc', epochs=20, 
gpus='0,1,2,3', log_interval=100, lr=0.001, lr_decay=0.1, lr_decay_epoch='14,20', 
lr_warmup=-1, max_size=1000, momentum=0.9, network='resnet50_v1b', num_workers=4, resume='', 
save_interval=1, save_prefix='faster_rcnn_resnet50_v1b_voc', seed=233, short=600, start_epoch=0, 
val_interval=1, verbose=False, wd=0.0005)
'''


class VOCLike(VOCDetection):
    # CLASSES =['tank', 'halter', 'romper', 'flannel', 'jersey', 'chinos', 'leggings', 'sweatpants', 'caftan', 'kaftan', 'jeans', 'henley', 'jeggings', 'capris', 'blouse', 'culottes', 'joggers', 'tee', 'button-down', 'turtleneck', 'gauchos', 'sweatshorts', 'bomber', 'cutoffs', 'blazer', 'skirt', 'kimono', 'jodhpurs', 'trunks', 'cardigan', 'sweater', 'jacket', 'hoodie', 'sarong', 'peacoat', 'onesie', 'shorts', 'top', 'dress', 'parka', 'coverup', 'poncho', 'jumpsuit', 'robe', 'anorak', 'coat']
    ...
    def _load_label(self, idx):
        ...
        for obj in root.iter('object'):
            ...
            if cls_name not in self.classes:
                label.append([0,0,width,height,-1,difficult])
...
def parse_args():
    ...
    parser.add_argument('--num-workers', '-j', dest='num_workers', type=int,
                        default=20, help='Number of data workers, you can use larger '
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        train_dataset = VOCLike(root='clothes_data',splits=((2018, 'train'),))
        print(train_dataset)
...
def save_params(net, logger, best_map, current_map, epoch, save_interval, prefix):
    ...
    if current_map > best_map[0]:
        ...
        net.save_parameters('./save_parameters/{:s}_best.params'.format(prefix))
...
if __name__ == '__main__':
    ...
    net = get_model(net_name, pretrained=True,per_device_batch_size=args.batch_size // len(ctx), **kwargs)
    ...
    train_data, val_data = get_dataloader(net, train_dataset, val_dataset, FasterRCNNDefaultTrainTransform,FasterRCNNDefaultValTransform, batch_size, len(ctx), args)
    # training
